...
5. **Nonlinear solution at the book parameters** (initial values = closed-form
   equilibrium (8.25) × 1.05): sV, sC, δ and τ oscillate with **median periods
   of ~49–51 y** over a 250-year window (6 eigenvalue periods; the equilibrium
   is an unstable focus, so amplitudes grow and the peak spacing lengthens
   from ~43 y to ~61 y — "about 50 years" is a median over the first cycles,
   not a stationary cycle), while the rate of profit r trends downwards — see
   `figures/ode_solution_book_params.png` (notes on the figure) and
...
2. `figures/ode_solution_book_params.png` — cycles of the **nonlinear solution
   ...
   (5 panels: r, sV, sC, δ, τ over 250 y = 6 eigenvalue periods, the same
   window as the validation doc; initial values = closed-form equilibrium
   (8.25) × 1.05; measured median period and rise/fall annotated per panel;
   a footnote explains why the oscillation is not a clean stationary ~50-y
   cycle).
...
| `figures/ode_solution_book_params.png` | Nonlinear ODE solution (r, sV, sC, δ, τ) at the book parameters over 250 y (= 6 eigenvalue periods): sV/sC/δ/τ median cycles ~49–51 y (amplitudes grow, spacings lengthen — see note on the figure), r trends down | Model equations (8.15)–(8.19) of the book; parameters as printed (no empirical data) |
